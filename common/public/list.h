...
#define DECLARE_LIST(name, type)                                               \
  ...
  /* Reserves memory for a total of `num_elems' objects, and increases the     \
   * total count. Capacity always increases by a factor of 2. Returns whether  \
   * the allocation was successful. */                                         \
  bool name##List_expand(name##List *self, size_t num_elems);                  \
  ...
  type *name##List_add_range(name##List *self, const type *elems,              \
                             size_t count);                                    \
  /* Truncates the list starting from `start_index'. */                        \
  void name##List_truncate(name##List *self, size_t start_index);              \
  /* Removes an element from the list. */                                      \
...
#define DEFINE_LIST(name, type)                                                \
  ...
  bool name##List_expand(name##List *self, size_t num_elems) {                 \
    return List_expand((List *)self, num_elems);                               \
  }                                                                            \
  ...
  void name##List_truncate(name##List *self, size_t start_index) {             \
    List_truncate((List *)self, start_index);                                  \
  }                                                                            \
  void name##List_remove(name##List *self, size_t start_index) {               \
